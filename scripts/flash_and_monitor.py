...
Usage:
  ...
  scripts/flash_and_monitor.py --env esp32-s3_debug   # different env
  scripts/flash_and_monitor.py --tail-grep "rosterAdd|jackDead"   # only print matching lines

Defaults to the esp32-s3_release_verbose pio environment. Devices are
...
PIO_ENV_DEFAULT = "esp32-s3_release_verbose"
PIO_BIN = "/usr/bin/pio"
...
def build_firmware(env):
    ...
    # Print the final summary line only
...
def flash_all(env, ports):
    # Try parallel multi_flash first — flashes all devices concurrently in
    # ~30s vs. ~2min sequential. Falls back to sequential if multi_flash fails
    # (older pio target, missing esptool in chosen python, etc).
...
def reader_thread(port, idx, duration, tail_filter):
    ...
    try:
        ...
        # Fall back to spawning the venv python as a subprocess.
...
def main():
    ...
    ap.add_argument("--env", default=PIO_ENV_DEFAULT,
                    help="platformio env name (default: esp32-s3_release_verbose)")
    ...
    # Main wait. If duration is 0, wait until Ctrl-C / _stop.
    if args.duration:
