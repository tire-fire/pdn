...
public:
    ...
    void exec() override {
        // Drain pending raw bytes to the byte callback. When fragmentation is
        // enabled, deliver only 1-3 bytes per tick so byte-stream parsers see
        // the same partial-delivery behavior they face on real UART hardware.
        if (pendingBytes_.empty()) {
            return;
        }
        size_t chunkSize = pendingBytes_.size();
        if (fragmentDeliveries_) {
            // Deterministic 1-3 byte cycle: 1, 2, 3, 1, 2, 3 ...
