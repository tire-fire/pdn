...
private:
    ...
    /**
     * Transfer raw bytes from one jack's output buffer into the peer jack's RX
     * path. Presence/topology is binary HELLO/BEACON framing, so the bytes are
     * routed verbatim into the peer's byte demuxer via injectInputBytes — no
     * line splitting or ASCII reframing.
     */
