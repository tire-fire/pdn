...
inline void hwmClearCallbacksRemovesAll(HWMUnitTests* suite) {
    ...
    RawPacket pkt{ static_cast<int>(SerialIdentifier::OUTPUT_JACK), static_cast<int>(SerialIdentifier::INPUT_JACK), 0, 1, HSCommand::EXCHANGE_ID };
    suite->hwm.processHandshakeCommand(dummyMac, reinterpret_cast<const uint8_t*>(&pkt), sizeof(pkt));

    RawPacket pkt2{ static_cast<int>(SerialIdentifier::INPUT_JACK), static_cast<int>(SerialIdentifier::OUTPUT_JACK), 0, 1, HSCommand::EXCHANGE_ID };
...
inline void hwmProcessRejectsNegativeCommand(HWMUnitTests* suite) {
    ...
    struct RawPacket { int sendingJack; int receivingJack; int deviceType; int gameRole; int command; } __attribute__((packed));
    RawPacket pkt{ static_cast<int>(SerialIdentifier::OUTPUT_JACK), static_cast<int>(SerialIdentifier::INPUT_JACK), 0, 1, -1 };
    uint8_t dummyMac[6] = {};
